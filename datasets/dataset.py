...
class Crowd(data.Dataset):
    def __init__(self, root_path, crop_size=512,
                 downsample_ratio=8, is_gray=False,
                 phase='train', resize=False, im_list=None, noise=0):
        ...
        self.phase = phase
        if self.phase == 'train':
            self.im_list *= 4  # augmentation for training data
    ...
    def __getitem__(self, item):
        ...
        if self.phase == 'train':
            ...
            return img, torch.from_numpy(keypoints.copy()).float()
    ...
    def train_transform_with_crop(self, img, keypoints):
        ...
        if st_size < self.c_size:
            c_size = 832  # min size in resized dataset
        else:
            c_size = self.c_size
        assert st_size >= self.c_size  # min size of original img has to be >= crop size
