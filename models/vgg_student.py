...
class VGG19Student(nn.Module):
    def __init__(self, ratio=4, transform=True):
        super(VGG19Student, self).__init__()
        ...
        self.conv4_4 = conv_layers(channel[3], channel[2])
        if transform:
            self.transform4_4 = feature_transform(channel[2], 256)
        self.conv4_5 = conv_layers(channel[2], channel[1])

        self.conv5_0 = nn.Conv2d(channel[1], 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = self.conv4_4(x)
        if self.transform:
            self.features.append(self.transform4_4(x))
        x = self.conv4_5(x)
...
def pool_layers(ceil_mode=False):
